...
DRIVING_PROCESS DriveToDestination::AnaliseDrivingProcess()
{
...
void DriveToDestination::BeginDrivingProcess(bool returnToOwner)
{
	...
	if (buildingOwner->GetPatchRoad()->aStarSearch(B, A))
	{
		...
		{
			if (this->buildingOwner->hasVehicle == true)
...
DRIVING_PROCESS DriveToDestination::MakeProgress()
{
	...
	{
		this->drivingProcess = DRIVING_PROCESS::AT_DESTINATION;
		//	std::cout << "At destination afrer MAKEPROFRESS" << std::endl;
	}
...
{
	this->buildingOwner->hasVehicle = false;
}
...
DRIVING_PROCESS DriveToDestination::GetDrivingProcess() const
{
	return this->drivingProcess;
}
