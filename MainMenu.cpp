...
void MainMenu::UpdateObject(float elapsedTime)
{
	...
	{
		ptrGame->stateMachine.NewState(std::unique_ptr<StateOfProgram>(new StartNewGame(ptrGame)), "StartNewGame", true);
	}

	if (this->mapButtons["LOAD GAME"]->GetPressed())
	{
		ptrGame->stateMachine.NewState(std::unique_ptr<StateOfProgram>(new LoadGame(ptrGame)), "LoadGame", true);
	}

	if (this->mapButtons["OPTIONS"]->GetPressed())
	{
		ptrGame->stateMachine.NewState(std::unique_ptr<StateOfProgram>(new Options(ptrGame)), "Options", true);
	}
