...
void MapGenerator::GenerateMapBaseOnSeed(unsigned int n)
{
	...
	for (int i = 0; i < n; i++)
	{
		for (int j = 0; j < n; j++)
		{
			basicTileMap[i*n + j] = TileSpecialise::FLOOR;

			if (generatedMap[i*n + j] < treeChance)
			{
			basicTileMap[i*n + j] = TileSpecialise::ROCK;
			}
			else if (generatedMap[i*n + j] < waterChance)
			{
				basicTileMap[i*n + j] = TileSpecialise::WATER;
			}
			else if (generatedMap[i*n + j] > rockChance)
			{
			basicTileMap[i*n + j] = TileSpecialise::TREE;
			}
